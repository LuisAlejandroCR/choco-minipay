...
// ChocoGateway-specific
const ledger      = process.env.VITE_LEDGER_ADDRESS     || process.env.LEDGER_ADDRESS || ethers.ZeroAddress;
const feeRecipient = process.env.FEE_RECIPIENT_ADDRESS;
const feeBps      = Number(process.env.FEE_BPS ?? "3");  // default 3 = 0.03%

if (!feeRecipient) throw new Error("Set FEE_RECIPIENT_ADDRESS — the wallet that receives the 0.03% protocol fee.");
if (feeBps < 0 || feeBps > 100) throw new Error("FEE_BPS must be 0–100 (0% to 1%).");

console.log("\n🚀 Deploying ChocoGateway to Celo Mainnet");
console.log("=".repeat(60));
console.log("Mento Broker:    ", broker);
console.log("ChocoLedger:     ", ledger === ethers.ZeroAddress ? "(none — logging disabled)" : ledger);
console.log("Fee recipient:   ", feeRecipient);
console.log(`Protocol fee:     ${feeBps} bps (${(feeBps / 100).toFixed(2)}%)`);
console.log("=".repeat(60) + "\n");
...
const contract = await factory.deploy(
  broker, provider, usdcToUsdm, usdmToCkes,
  usdc, usdm, ckes,
  ledger,
  feeRecipient,
  feeBps
...
const gatewayAddress = await contract.getAddress();
const deployBlock    = await rpc.getBlockNumber();

console.log("✅ ChocoGateway:  ", gatewayAddress);
console.log("   Deploy block:  ", deployBlock);

if (ledger !== ethers.ZeroAddress) {
  console.log("\n⚡ Authorize ChocoGateway on ChocoLedger (run as admin):");
  console.log(`   cast send ${ledger} "setSwapContract(address,bool)" ${gatewayAddress} true \\`);
  console.log(`     --rpc-url ${rpcUrl} --private-key $DEPLOYER_PRIVATE_KEY`);
}

console.log("\n📋 Add to Vercel environment variables:");
console.log(`VITE_CKES_SWAP_CONTRACT_ADDRESS=${gatewayAddress}`);
console.log(`VITE_CKES_SWAP_DEPLOY_BLOCK=${deployBlock}`);
